...
| e2e                  | Playwright (added by S-03)                                       | 1.60.x                   | Not warranted for the core one-page tool. S-03 (passphrase-access-gate) introduced the project's first browser-level risk — edge middleware redirect + session cookie + SSR + island mount — which only manifests in the running app; covered by `src/test/passphrase-gate.e2e.spec.ts`. See that slice's plan. Re-evaluate §7 SSR/plumbing exclusion accordingly. |
